...
def get_std_dev(mk_geo_radcal2_log):
    with open(mk_geo_radcal2_log) as f:
        line = [
            ...
            if line.startswith("final model fit std. dev. (samples)")
        ][0]
...
def get_offset(diff_par):
    ...
    azimuth_offset = float(
        getParameter(diff_par, "azimuth_offset_polynomial").split()[0]
    )
...
def check_coregistration(
    mk_geo_radcal2_log, diff_par, max_stddev=2.0, max_offset=75, pixel_size=30.0
):
