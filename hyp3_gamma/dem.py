...
def shift_for_antimeridian(dem_file_paths: List[str], directory: Path) -> List[str]:
    ...
    for file_path in dem_file_paths:
        if "_W" in file_path:
            shifted_file_path = str(
                directory / Path(file_path).with_suffix(".vrt").name
            )
            ...
            gdal.Translate(
                shifted_file_path, file_path, format="VRT", outputBounds=output_bounds
            )
...
def prepare_dem_geotiff(
    output_name: str, geometry: ogr.Geometry, pixel_size: float = 30.0
):
    ...
    with GDALConfigManager(GDAL_DISABLE_READDIR_ON_OPEN="EMPTY_DIR"):
        if not intersects_dem(geometry):
            raise DemError(
                f"Copernicus GLO-30 Public DEM does not intersect this geometry: {geometry}"
            )
