...
def get_height_at_pixel(
    in_height_file: str, mlines: int, mwidth: int, ref_azlin: int, ref_rpix: int
) -> float:
...
def coords_from_sarpix_coord(
    in_mli_par: str, ref_azlin: int, ref_rpix: int, height: float, in_dem_par: str
) -> list:
    """
...
def get_coords(
    ...
    """
    row_s, col_s, lat, lon = coords_from_sarpix_coord(
        in_mli_par, ref_azlin, ref_rpix, height, "-"
    )
    coords = {"row_s": int(row_s), "col_s": int(col_s), "lat": lat, "lon": lon}

    if in_dem_par:
        _, _, _, _, y, x = coords_from_sarpix_coord(
            in_mli_par, ref_azlin, ref_rpix, height, in_dem_par
        )
        coords["y"] = y
        coords["x"] = x
...
def get_reference_pixel(
    coherence: np.array, window_size=(5, 5), coherence_threshold=0.3
) -> Tuple[int, int]:
    """
    Args:
...
def get_water_mask(cc_file, width, lt, demw, demn, dempar):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        data2geotiff(
            f"{temp_dir}/tmp_mask_geo", f"{temp_dir}/tmpgtiff_mask_geo.tif", dempar, 2
        )
...
def unwrapping_geocoding(
    ...
    if apply_water_mask:
        ...
        out_file = combine_water_mask(
            f"{ifgname}.adf.cc_mask.bmp", "water_mask_sar.bmp"
        )
        # apply water mask in SAR space to cc
        cc_ref = apply_mask(
            f"{ifgname}.cc", int(mlines), int(mwidth), "water_mask_sar.bmp"
        )
    ...
    height = get_height_at_pixel(
        f"DEM/HGT_SAR_{rlooks}_{alooks}", int(mlines), int(mwidth), ref_azlin, ref_rpix
    )
    ...
    geocode_back(
        "{}.adf".format(ifgf), "{}.adf.geo".format(ifgf), width, lt, demw, demn, 1
    )
    geocode_back(
    ...
    geocode_back(
        "{}.cc".format(ifgname), "{}.cc.geo".format(ifgname), width, lt, demw, demn, 0
    )
    geocode_back(
    ...
    create_phase_from_complex(
        "{}.adf.geo".format(ifgf), "{}.adf.geo.phase".format(ifgf), width
    )
    ...
    data2geotiff(
        "{}.adf.geo.phase".format(ifgf), "{}.adf.geo.tif".format(ifgf), dempar, 2
    )
    data2geotiff(
        "{}.adf.bmp.geo".format(ifgf), "{}.adf.bmp.geo.tif".format(ifgf), dempar, 0
    )
    data2geotiff(
        "{}.cc.geo".format(ifgname), "{}.cc.geo.tif".format(ifgname), dempar, 2
    )
    data2geotiff(
        "{}.adf.cc.geo".format(ifgname), "{}.adf.cc.geo.tif".format(ifgname), dempar, 2
    )
...
def main():
    ...
    parser.add_argument(
        "-r", "--rlooks", default=10, help="Number of range looks (def=10)"
    )
    parser.add_argument(
        "-a", "--alooks", default=2, help="Number of azimuth looks (def=2)"
    )
    parser.add_argument(
        "-t",
        "--tri",
    ...
    parser.add_argument(
        "--alpha", default=0.6, type=float, help="adf filter alpha value (def=0.6)"
    )
