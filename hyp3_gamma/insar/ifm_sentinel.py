...
def make_parameter_file(
    ...
    log.info(
        "In directory {} looking for file with date {}".format(
            os.getcwd(), reference_date_short
        )
    )
    ...
    with open(parameter_file_name, "w") as f:
        ...
        f.write(
            "Azimuth line of the reference point in SAR space: %s\n" % coords["row_s"]
        )
        f.write(
            "Range pixel of the reference point in SAR space: %s\n" % coords["col_s"]
        )
        f.write(
            "Y coordinate of the reference point in the map projection: %s\n"
            % coords["y"]
        )
        f.write(
            "X coordinate of the reference point in the map projection: %s\n"
            % coords["x"]
        )
...
def insar_sentinel_gamma(
    ...
    dem_pixel_size = (
        int(alooks) * 40
    )  # typically 160 or 80; IFG pixel size will be half the DEM pixel size (80 or 40)
    ...
    os.chdir(secondary)
    SLC_copy_S1_fullSW(
        wrk, secondary, "SLC_TAB", burst_tab2, mode=2, raml=rlooks, azml=alooks
    )
    os.chdir("..")
    ...
    interf_pwr_s1_lt_tops_proc(
        reference, secondary, hgt, rlooks=rlooks, alooks=alooks, iterations=3, step=0
    )

    log.info("Starting interf_pwr_s1_lt_tops_proc.py 1")
    interf_pwr_s1_lt_tops_proc(
        reference, secondary, hgt, rlooks=rlooks, alooks=alooks, step=1
    )

    log.info("Starting interf_pwr_s1_lt_tops_proc.py 2")
    interf_pwr_s1_lt_tops_proc(
        reference, secondary, hgt, rlooks=rlooks, alooks=alooks, iterations=3, step=2
    )
    ...
    interf_pwr_s1_lt_tops_proc(
        reference, secondary, hgt, rlooks=rlooks, alooks=alooks, step=3
    )
    ...
    product_name = get_product_name(
        reference_file, secondary_file, orbit_files, pixel_spacing, apply_water_mask
    )
...
def main():
    ...
    parser.add_argument(
        "-r", "--rlooks", default=20, help="Number of range looks (def=20)"
    )
    parser.add_argument(
        "-a", "--alooks", default=4, help="Number of azimuth looks (def=4)"
    )
    parser.add_argument(
        "-d", action="store_true", help="Add DEM file to product bundle"
    )
    parser.add_argument(
        "-i",
        action="store_true",
    ...
    parser.add_argument(
        "-l", action="store_true", help="Create look vector theta and phi files"
    )
    parser.add_argument(
        "-s",
        action="store_true",
