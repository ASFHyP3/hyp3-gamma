...
class InSarMetadataWriter:
    ...
    def create_look_vector_xmls(self) -> List[Path]:
        ...
        output_files = [
            self.create_metadata_file(
                self.payload, "insar/lv_phi_tif.xml.j2", reference_file_phi
            ),
            self.create_metadata_file(
                self.payload, "insar/lv_theta_tif.xml.j2", reference_file_theta
            ),
        ]
        return output_files
    ...
    def create_browse_xmls(self) -> List[Path]:
        ...
        output_files = [
            self.create_metadata_file(
                self.payload, "insar/color_phase_png.xml.j2", reference_file_col
            ),
            self.create_metadata_file(
                self.payload, "insar/unw_phase_png.xml.j2", reference_file_unw
            ),
        ]
        return output_files
    ...
    def create_metadata_file(
        ...
        if reference_file.suffix == ".png":
            payload["thumbnail_encoded_string"] = util.get_thumbnail_encoded_string(
                reference_file
            )
        else:
...
def marshal_metadata(
    ...
    payload = locals()
    payload["granule_type"] = util.get_granule_type(reference_granule_name)[
        "granule_type"
    ]
...
def populate_example_data(product_dir: Path):
    ...
    for f in product_files:
        shutil.copy(
            f, product_dir / f'{Path(f).name.replace("insar", product_dir.name)}'
        )
