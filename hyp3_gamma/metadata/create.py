...
def get_dem_blurb(dem_name: str) -> str:
    if dem_name.startswith('EU'):
        return 'The EU-DEM combines data from a variety of sources, including the ASTER DEM, the SRTM, and Russian ' \
               'topographic maps. For more information and to access the full EU-DEM dataset, refer to ' \
               'https://land.copernicus.eu/imagery-in-situ/eu-dem/eu-dem-v1.1.'
    if dem_name.startswith('GIMP'):
        return 'The GIMP DEM was constructed by combining ASTER and SPOT 5 DEMs over the ice sheet periphery and ' \
               'margin with AVHRR photoclinometry for the interior and far north, and calibrating the data to ' \
               'approximate mean ICESat/GLAS elevations from 2003 to 2009. For more information and to access the ' \
               'full GIMP dataset, refer to https://nsidc.org/data/nsidc-0645.'
    if dem_name.startswith('NED'):
        return 'The National Elevation Dataset (NED) provides the best available public domain raster elevation ' \
               'data of the conterminous United States, Alaska, Hawaii, and territorial islands in a seamless ' \
               'format. The NED is derived from diverse source data, processed to a common coordinate system and ' \
               'unit of vertical measure. For more information, refer to ' \
               'https://pubs.er.usgs.gov/publication/70201572. To download the data, visit ' \
               'https://viewer.nationalmap.gov/basic and expand the Elevation Products (3DEP) section.'
    if dem_name.startswith('REMA'):
        return 'The REMA DEM was constructed from hundreds of thousands of individual stereoscopic Digital Elevation ' \
               'Models extracted from pairs of submeter-resolution DigitalGlobe satellite imagery acquired between ' \
               '2009 and 2017, and vertically registered to altimetry measurements from Cryosat-2 and ICESat. For ' \
               'more information and to access the full REMA dataset at the original 8-meter resolution, refer to ' \
               'https://www.pgc.umn.edu/data/rema/.'
    if dem_name.startswith('SRTM'):
        return 'The SRTM was flown aboard the space shuttle Endeavour February 11-22, 2000. The National ' \
               'Aeronautics and Space Administration (NASA) and the National Geospatial-Intelligence Agency ' \
               '(NGA) participated in an international project to acquire radar data which were used to create ' \
               'the first near-global set of land elevations. For more information and to access the full SRTM ' \
               'dataset, refer to ' \
...
def marshal_metadata(product_dir: Path, granule_name: str, dem_name: str, processing_date: datetime, looks: int,
    ...
    payload['dem_blurb'] = get_dem_blurb(dem_name)
