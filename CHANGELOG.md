...
- Documentation for the adaptive phase filter parameter options in product readme and xml files
...
### Fixed
- [#491](https://github.com/ASFHyP3/hyp3-gamma/issues/491) `util.unzip_granule` fails with `StopIteration` for recently acquired Sentinel-1 scenes
...
### Changed
* Upgraded to [GAMMA software](https://www.gamma-rs.ch/software) version `20220630` from `20210701`
...
### Changed
* Upgraded to asf-tools [v0.3.2](https://github.com/ASFHyP3/asf-tools/blob/develop/CHANGELOG.md#032) from v0.3.1
...
### Fixed
* `asf-tools` conda environment, which is used by the water mapping code, has been (temporarily) locked to the environment as created
   in HyP3 GAMMA v5.1.2 to resolve some dependency issues.
...
* InSAR products now include an ellipsoidal incidence angle GeoTIFF (in addition to local incidence angle)
  when selecting the `include_inc_map` option
