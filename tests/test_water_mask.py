...
def test_get_corners(tmp_path, test_data_dir):
    ...
    corners_1 = np.round(
        np.asarray(water_mask.get_corners(filepath_1, tmp_path=str(tmp_path))), 13
    )
    filepath_2 = str(test_data_dir / "test_geotiff.tif")
    corners_2 = np.round(
        np.asarray(water_mask.get_corners(filepath_2, tmp_path=str(tmp_path))), 13
    )
    assert (
        corners_1.all()
        == np.round(
...
def test_create_water_mask(tmp_path, test_data_dir):
    ...
    water_mask.create_water_mask(
        input_image, output_image, gdal_format="ISCE", tmp_path=tmp_path
    )
