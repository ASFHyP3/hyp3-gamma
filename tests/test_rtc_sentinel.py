...
def test_get_product_name():
    ...
    assert match('S1A_S1_20181121T184017_SHP_RTC30_G_ueans_[0-9A-F]{4}$', name)
    ...
    name = rtc_sentinel.get_product_name(**payload)
    assert match('S1B_WV_20200714T162902_SVR_RTC10_G_uepfg_[0-9A-F]{4}$', name)
    ...
    name = rtc_sentinel.get_product_name(**payload)
    assert match('S1B_IW_20200714T152128_DVO_RTC30_G_uepng_[0-9A-F]{4}$', name)

    name = rtc_sentinel.get_product_name('S1A_EW_RAW__0SDH_20151118T190420_20151118T190529_008663_00C507_0A5F', None)
    assert match('S1A_EW_20151118T190420_DHO_RTC30_G_uepng_[0-9A-F]{4}$', name)
