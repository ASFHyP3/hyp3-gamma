...
def test_create_rtc_gamma_readme(rtc_product_dir):
    ...
    assert (
        output_file
        == rtc_product_dir
        / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2.README.md.txt"
    )
...
def test_create_dem_xml(rtc_product_dir):
    for dem_name in SUPPORTED_DEMS:
        ...
        assert (
            output_file
            == rtc_product_dir
            / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2_dem.tif.xml"
        )
        assert output_file.exists()
        output_file.unlink()
...
def test_rtc_gamma_area(rtc_product_dir):
    ...
    assert (
        output_file
        == rtc_product_dir
        / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2_area.tif.xml"
    )
    assert output_file.exists()
...
def test_rtc_gamma_inc_map(rtc_product_dir):
    ...
    assert (
        output_file
        == rtc_product_dir
        / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2_inc_map.tif.xml"
    )
    assert output_file.exists()
...
def test_rtc_gamma_ls_map(rtc_product_dir):
    ...
    assert (
        output_file
        == rtc_product_dir
        / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2_ls_map.tif.xml"
    )
    assert output_file.exists()
...
def test_rtc_gamma_rgb(rtc_product_dir):
    ...
    assert (
        output_file
        == rtc_product_dir
        / "S1A_IW_20150621T120220_DVP_RTC10_G_saufem_F8E2_rgb.tif.xml"
    )
    assert output_file.exists()
...
def test_thumbnail_reference_file_is_browse(test_data_folder):
    ...
    encoded_string = hyp3_gamma.metadata.util.get_thumbnail_encoded_string(
        reference_file
    )
